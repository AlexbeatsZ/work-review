...
import com.metacodex.workreview.data.usage.PackageVisualResolver
...
class WorkReviewRepository(private val context: Context) {
    ...
    private val packageVisuals = PackageVisualResolver(context)
    ...
    fun appVisual(packageName: String, fallbackLabel: String? = null): AppVisual =
        packageVisuals.resolve(packageName, fallbackLabel)

    suspend fun installedApps(): List<InstalledApp> = withContext(Dispatchers.IO) {
        val ignored = ignoredPackages()
        val knownPackages = dao.knownPackageNames().toSet() + ignored
        val packageManager = context.packageManager
        @Suppress("DEPRECATION")
        val installed = packageManager.getInstalledApplications(0)
            .asSequence()
            .filter { info ->
                knownPackages.contains(info.packageName) ||
                    packageManager.getLaunchIntentForPackage(info.packageName) != null
            }
            .associateBy { it.packageName }

        (installed.keys + knownPackages)
            .map { packageName ->
                val visual = packageVisuals.resolve(packageName)
                InstalledApp(
                    packageName = packageName,
                    label = visual.label,
                    icon = visual.icon,
                    isIgnored = ignored.contains(packageName)
                )
            }
            .sortedWith(
                compareBy<InstalledApp> { !it.isIgnored }
